...
(function () {
  ...
  const TOOL_ID = 'eisai-chatgpt-tool-v0-1-30';
  const BTN_ID = 'eisai-chatgpt-btn-v0-1-30';
  const STORAGE_KEY = 'eisai_chatgpt_blog_info_v0130';
...
- ノートや答案を見せる場合は、紙面がカメラから読める向きに傾いていることを指定してください。上下逆、鏡文字、横向きすぎる紙面は禁止です。
- 紙面上の日本語や数字は、見る人に対して正しい向きで、左から右・上から下に読めるようにしてください。紙の向きと文字の向きがズレないようにします。
...
物理整合性まで書く例：
「カメラは机の斜め上45度から。答案用紙は画面下から奥へ少し傾けて置き、紙面の数字と途中式は視聴者から読める向き。先生の右手のペン先は答案の途中式を指し、生徒と保護者の視線も同じ答案に向く。メイン文字は紙の上ではなく画面左側に水平に重ねる。」
...
- カメラ角度、紙面の向き、紙面上の文字方向、手やペンの向きが物理的に自然
- 記事にない実績や数字は作らない
