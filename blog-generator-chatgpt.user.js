...
(function () {
  ...
  const TOOL_ID = 'eisai-chatgpt-tool-v0-1-16';
  const BTN_ID = 'eisai-chatgpt-btn-v0-1-16';
  const STORAGE_KEY = 'eisai_chatgpt_blog_info_v0116';
  ...
  const THUMBNAIL_ART_DIRECTIONS = [
    'Score Impact: bold numeric result as the hero, dynamic diagonal text, supporting classroom or test paper detail, high energy but not cluttered',
    'Notebook Close-up: macro view of a notebook, written steps, pen, worksheet, hands in action, text placed in clean negative space',
    'Before After Split: split-screen composition showing before and after states, left side problem/frustration, right side improvement/result',
    'Parent Empathy: emotional parent concern theme, calm home-study or consultation atmosphere, softer colors, text focused on the pain point',
    'Student Story: student-centered change moment, expression or posture shows confidence, article scene becomes the visual anchor',
    'Answer Sheet Moment: test answer sheet, score marks, red pen, paper texture, result and reason shown as typography',
    'Teacher Support: teacher and student side-by-side only when it directly supports the article, natural guidance scene, not always white coat',
    'Infographic Editorial: clean diagram, arrows, timeline, checklist, score transition, minimal people, modern education magazine layout',
    'Event Announcement: calendar, date, class atmosphere, participation scene, structured text blocks suitable for campaign posts',
    'Character Spotlight: person introduction layout, portrait or uploaded photo as hero, name and personality cue, graphic background'
  ];

  const THUMBNAIL_LAYOUT_VARIANTS = [
    'left text block / right visual subject',
    'right text block / left visual subject',
    'top headline / bottom close-up object',
    'central object hero / diagonal text ribbon',
    'split-screen before-after',
    'magazine cover layout with small labels',
    'minimal object poster with large negative space',
    'collage layout with 2-3 evidence objects',
...
4. Text Design Direction: choose typography that fits the selected art direction. Do not always use huge 3D letters. Use bold 3D only when Score Impact or YouTube-like impact is selected. Otherwise use editorial, clean, handwritten note, label, ribbon, speech bubble, or infographic typography as appropriate.
...
- 大きな3D文字だけで画面を埋める構図を毎回使わないでください
- 同じ文言を複数回表示しないでください
